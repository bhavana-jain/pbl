...
const AllUserEntries = (props, ref) => {
	...
	let filterUnredeemed;

	filterUnredeemed = entries.filter((entry) => entry.redemptionDate == null || entry.redemptionDate == undefined || entry.redemptionDate == "");

	let selectYr; 
	const setYear = (selectYear) => {
		selectYr =  selectYear;
		filterUnredeemed = entries.filter((entry) => {
			return new Date(entry.date).getFullYear() == selectYr
		});
		console.log(filterUnredeemed);
		return filterUnredeemed;
		...
		<>
			...
			<div className="entry-content">
				...
				<ul className='year-tabs'>
					...
					<li onClick={() => setYear(2023)} >2023</li>
					<li onClick={() => setYear(2024)} >2024</li>
				</ul>
