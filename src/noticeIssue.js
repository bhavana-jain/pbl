...
const NoticeIssue = (props, ref) => {
	...
	const filterUnreedemedEntry = () => {
		// Filter all entries by current month and a year older than current year
		filterUnredeem = entries.filter((entry) => !entry.redemptionDate && new Date(entry.date).getMonth() == new Date().getMonth() && new Date(entry.date).getFullYear() == parseInt(new Date().getFullYear() - 1));
