...
const EditEntryModal = (props) => {
	...
	const postUpdatedData = async (e) => {
		...
		e.preventDefault();
		await axios.put('http://localhost:4000/customers/update-student/' + props.toEdit, { ...inputVal, "redemptionAmount":redemptionAmt.current.value }, { headers: { 'Content-Type': 'application/json' } })
			.then(res => console.log(res.data));
	}
				...
				{inputVal ?
					<form className="entry-form">
						...
						<div>
							...
							<input type="text" name="address" placeholder="Enter address" value={inputVal.address} onChange={handleChange} readOnly={toDeliver} />
						</div>
						<div>
